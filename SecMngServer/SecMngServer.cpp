#include"SecMngServer.h"

SecMngServer::SecMngServer(muduo::net::EventLoop* loop, const muduo::net::InetAddress& listenAddr)
    : _server(loop, listenAddr, "SecMngServer"),
      _loop(loop)
{
    _server.setConnectionCallback(std::bind(&SecMngServer::onConnection, this, std::placeholders::_1));
    _server.setMessageCallback(std::bind(&SecMngServer::onMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
SecMngServer::~SecMngServer()
{

}

void SecMngServer::start()
{
    _server.start();
}

void SecMngServer::onConnection(const muduo::net::TcpConnectionPtr& conn){
    if (conn->connected())
    {
...
bool SecMngServer::check_sign(const Request&req){
    std::string signData;
...
void SecMngServer::handle_keyrevoke(const Request&req,Response&rsp){
...
void SecMngServer::handle_keyagree(const Request&req,Response&rsp){
    ...
    //写入共享内存
    SecKeyInfo secinfo;
    memset(secinfo.clientId,0,sizeof(secinfo.clientId));
    memcpy(secinfo.clientId,req.clientid().data(),req.clientid().size());
    time_t cur_time=time(NULL);
    secinfo.createTime=cur_time;
    secinfo.expireTime=cur_time+3600;
    memcpy(secinfo.secKey,_aesMap[req.clientid()]->getKey(),KEY_SIZE);
    secinfo.secKeyId=static_cast<uint>(stoul(req.seckeyid()));
    secinfo.serverId=static_cast<uint>(stoul(req.serverid()));
    secinfo.used=true;
    if(_secshm->find(req.clientid().data())){
        if(!_secshm->update(secinfo)){
            rsp.set_status(false);
            return;
        }
    }
...
void SecMngServer::handle_keycheck(const Request&req,Response&rsp){
...
void SecMngServer::onMessage(const muduo::net::TcpConnectionPtr& conn, muduo::net::Buffer* buf, muduo::Timestamp time){
